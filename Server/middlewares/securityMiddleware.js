...
const validateInput = (req, res, next) => {
  ...
  const checkForSuspiciousInput = (obj, path = '') => {
    if (typeof obj === 'string') {
      for (const pattern of suspiciousPatterns) {
        if (pattern.test(obj)) {
          console.warn(`🚨 Suspicious input detected at ${path}:`, obj.substring(0, 100));
  ...
  const result = checkForSuspiciousInput(req.body, 'body') ||
    ...
    for (const file of files) {
      ...
      if (file.mimetype && allowedTypes[file.mimetype]) {
        ...
        if (!validExtensions.includes(fileExtension)) {
          console.warn(`🚨 File extension mismatch: ${file.originalname} (${file.mimetype})`);
          return res.status(400).json({
            success: false,
